...
cluster_plot_data <- cbind(
  train_data,
  pca_3
)

cluster_plot_data$cluster <- as.factor(cluster_plot_data$cluster)

ggplot(cluster_plot_data, aes(x = PC1, y = PC2, color = cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "Consumer Clusters Visualized Using PCA Components",
...
ggplot(train_data, aes(x = as.factor(cluster), y = IncomeImp, fill = as.factor(cluster))) +
  geom_boxplot() +
  labs(
  ...
  theme_minimal()
centers_df <- as.data.frame(cluster_model$centers)
...
ggplot(centers_long, aes(x = variable, y = value, fill = cluster)) +
  ...
  labs(title = "Cluster Centers Comparison") +
  theme_minimal() +
...
reg_test <- cbind(test_data_std[,reg_vars], test_pca_3)
...
print(table(train_balanced$AcceptedAny))
cat("\nProportion:\n")
print(prop.table(table(train_balanced$AcceptedAny)))
